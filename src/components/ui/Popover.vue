...
<template>
	<div class="relative mx-auto w-fit pb-3" @mouseleave="isOpen = false">
		<!-- component to render the popover on -->
		<div class="relative w-fit" @mouseover="isOpen = true">
			<!-- arrow icon -->
		...
		<!-- popover content -->
		<div v-if="isOpen" :class="{ 'left-0': position === 'left', 'right-0': position === 'right', 'left-1/2 -translate-x-1/2': position === 'center', bodyClass }" class="absolute z-10 mt-3 w-[400px] rounded-md border border-gray-200 bg-white p-3 shadow-lg">
			<slot name="body" />
		</div>
	</div>
</template>
