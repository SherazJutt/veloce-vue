...
<template>
  ...
  <div :class="{ shadow: props.shadow }" class="bg-background w-full divide-y overflow-hidden rounded border">
    ...
    <div v-for="(item, index) in items" :key="index" v-if="items.length" class="text-sm">
      ...
      <div class="relative flex cursor-pointer justify-between gap-4 p-3 font-medium duration-100 hover:bg-neutral-100 dark:hover:bg-neutral-800" @click="activeIndex = activeIndex === index ? null : index">
        <!-- Accordion Title -->
      ...
      <!-- Accordion Body -->
      <div v-if="activeIndex === index" class="p-3">
